...
public:
  ...
  void SendNeighborInfoToGPU(void) {
    ...
    mesh->TransposeSortedList(vars);
    const auto max_partner_number = *std::max_element(number_of_partners,
                                                      number_of_partners + pn);
    const auto tlist_size = max_partner_number * pn;
    #pragma acc update device(number_of_partners[0:pn], transposed_list[0:tlist_size])
  }
#endif
